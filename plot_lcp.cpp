...
for(size_t function_counter = 0; function_counter < lcp_functions; ++function_counter) {
  ...
  os_gnu << "'" <<  argv[2] << "' using 1:(($" << min_line << "+$" << max_line << ")/2):($" << max_line << "-$" << min_line << ") with yerrorbars title '" << lcp_name[function_counter] << "', ";
}
  const size_t min_line = (lcp_functions*2+1);
  const size_t max_line = (lcp_functions*2+2);
...
os_gnu << std::endl;
  ...
  std::ofstream os_plot(argv[2]);
    os_plot << "##length";
  for(size_t function_counter = 0; function_counter < lcp_functions; ++function_counter) {
    os_plot << '\t' << lcp_name[function_counter] << "_min" << '\t' << lcp_name[function_counter] << "_max";
  }
